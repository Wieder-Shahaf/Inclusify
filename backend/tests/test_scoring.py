...
def test_no_findings_is_100():
    assert clean_text_score(1000, []) == (100, 0)


def test_empty_text_is_100():
    assert clean_text_score(0, [(0, 5)]) == (100, 0)
...
def test_integer_floor():
    # 1.333..% flagged -> 98.66 clean -> floors to 98 (never rounds up)
    score, flagged = clean_text_score(3000, [(0, 40)])
    assert flagged == 40
    assert score == 98


def test_single_flagged_char_never_shows_100():
    score, flagged = clean_text_score(1_000_000, [(0, 1)])
    assert flagged == 1
    assert score == 99
