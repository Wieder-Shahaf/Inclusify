#!/usr/bin/env python3
"""Phase 3 — H100 LoRA training, production-format SFT with completion-only loss.

Trains Qwen2.5-3B + LoRA on the curated production-format data (data/curated/*.jsonl).
Key changes vs the T4-era train_qwen_grid.py:
  - bf16 base, NO 4-bit quantization (3B fits in 80GB).
...
Run pinned to a single GPU:  CUDA_VISIBLE_DEVICES=1 python ml/training/train_h100.py
"""
import json
import os
import sys
...
def main():
    ...
    all_meta = []
    for rank, alpha, dropout in GRID_H100:
        try:
